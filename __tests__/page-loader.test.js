...
test('Debería lanzar un error si la solicitud HTTP principal falla', async () => {
  ...
  await expect(downloadPage(urlThatFails, tempDir)).rejects.toThrow(`Request failed with status code 500`);
...
test('Debería lanzar un error si no puede guardar el archivo HTML', async () => {
  ...
  await expect(downloadPage(urlToDownload, outputDir)).rejects.toThrow(`EACCES: permission denied, mkdir '/root/codica-la-cursos_files'`);  
...
test('Debería lanzar un error si no existe el directorio de recursos', async () => { 
  ...
  await expect(downloadPage(urlToDownload, tempDir)).rejects.toThrow(`Simulated mkdir permission error`);  
...
test('Debería lanzar un error si el directorio de salida no existe', async () => {
  ...
  await expect(downloadPage(urlToDownload, nonExistentDir)).rejects.toThrow(`ENOENT: no such file or directory, mkdir '${nonExistentDir}/example-com-page_files'`);
